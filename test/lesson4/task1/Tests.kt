...
class Tests {
    ...
    fun roman() {
        ...
        assertEquals("MMMCMLXXX", roman(3980))
    }

    @Test
    @Tag("Impossible")
    fun russian() {
        assertEquals("семьсот одна тысяча двести шестьдесят один", russian(701261))
        ...
        assertEquals("девятьсот сорок тысяч девятьсот девяносто два", russian(940992))
        assertEquals("шестьсот двадцать семь тысяч шестьсот пятьдесят один", russian(627651))
        assertEquals("восемьсот восемьдесят одна тысяча шестьсот восемьдесят один", russian(881681))
    }
}