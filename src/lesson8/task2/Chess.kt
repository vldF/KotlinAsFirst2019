...
data class Square(val column: Int, val row: Int) {
     ...
     */
    fun notation(): String = if (!inside()) "" else "${"abcdefgh"[column - 1]}${row}"
}
...
fun square(notation: String): Square =
    if (notation[0] in 'a'..'h' && notation[1].isDigit() && notation.length == 2) Square(
        notation[0] - 'a' + 1,
        notation[1] - '0'
...
fun rookTrajectory(start: Square, end: Square): List<Square> {
    ...
    if (start.row != end.row && start.column != end.column) {
        res.add(Square(end.column, start.row))
    }
    res.add(end)
    return res
}
...
fun bishopTrajectory(start: Square, end: Square): List<Square> {
    ...
    return when (bishopMoveNumber(start, end)) {
        ...
        1 -> res + end
            ...
            // Выглядит не очень, но это лучше, чем формулы на две строки каждая
            // Суть: задаю диагонали, по которым могут ходить слоны, при помощи двух уравнений:
            // y=x+k и y=-x+k (это два разных k в коде)
            // И решаю две системы уравнений. Если одно решение лежит на доске, то возвращаю его.
            // Если нет, то возвращаю другое
            val k1 = start.row - start.column
            val l1 = end.row + end.column
            val x1 = (l1 - k1) / 2
            val y1 = x1 + k1
            val k2 = start.row + start.column
            val l2 = end.row - end.column
            val x2 = (k2 - l2) / 2
            val y2 = x2 + l2

            if (Square(x1, y1).inside()) res + Square(x1, y1) + end
            else res + Square(x2, y2) + end
        }
    }
}
